...
[SRFI-174](https://srfi.schemers.org/srfi-174/) POSIX timespecs: a small, immutable `(seconds nanoseconds)` time-instant type shared across SRFIs that need one (e.g. [SRFI-19](https://srfi.schemers.org/srfi-19/) time/date), so they don't each define their own incompatible variant. Pure Scheme, no C.
...
- [`s19.md`](s19.md) — time/date objects, a natural pairing with this timespec type
