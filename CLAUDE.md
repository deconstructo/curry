...
- **Raw GLSL shaders** (`gfx-mandelbrot-gpu!`): The module supports raw OpenGL 3.3 Core Profile fragment shaders via `beginNativePainting()` / `endNativePainting()`. The context is requested as Core Profile 3.3 at startup (giving 4.1 on macOS, 3.3–4.6 on Linux depending on driver). HiDPI is handled by scaling the viewport and zoom by `devicePixelRatioF()`. `gfx-mandelbrot-gpu! painter cx cy zoom max-iter algebra theta phi` renders a full-resolution hypercomplex Mandelbrot set (complex/quaternion/octonion) entirely on the GPU. After `endNativePainting()`, QPainter is restored and `gfx-draw-text!` etc. work normally for HUD overlays.
