...
A `piper-speak-async` handle stays valid indefinitely after `piper-wait` returns — same as a `(curry posix)` process handle staying valid after `process-wait`, which is exactly the behavior `(curry tts)`'s own `tts-stop`/`tts-wait`/`tts-speaking?` dispatch relies on (it calls all three on the same handle in sequence for every backend, piper included). There is no `piper-handle-free!`: handles are reclaimed automatically at process exit instead, so ordinary use never needs to think about it. (An earlier version of this module freed the handle's backing struct inside `piper-wait` itself; that broke the exact `tts-stop`+`tts-wait`+`tts-speaking?` sequence above with a use-after-free, caught by `piper_tests.scm`, and was replaced with exit-time reclamation instead.)

**`piper-synth`, by contrast, still needs an explicit `piper-free!`** — the same "must be released, no GC finalizer" contract as before. Skipping it isn't just a leak, either: `libpiper` depends on `onnxruntime`, whose process-global `OrtEnv` singleton crashes the whole process (`SIGABRT`) at exit if any `piper_synthesizer` is still alive when its own static destructor runs. This module backstops that with an `atexit()`-registered sweep that force-frees any synth (and any finished playback handle) still outstanding, so forgetting `piper-free!`/`piper-wait` degrades to an ordinary leak instead of a guaranteed crash — but calling `piper-free!` promptly is still the correct, recommended usage.
