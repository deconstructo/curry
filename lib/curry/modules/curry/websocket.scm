...
(define-library (curry websocket)
  ...
  (begin
    ...
    ;;
    ;; Optional timeout-ms (issue #237, made genuinely race-free by
    ;; issue #238): with no timeout, this blocks indefinitely -- fine
    ;; for a long-lived server, but a real footgun for anything that
    ;; needs to notice "no client ever showed up" (a test, a CLI tool
    ;; with a startup grace period, ...), since the only thing that
    ;; would ever catch a stuck accept is something entirely external to
    ;; this call, like a blunt process-level timeout with no diagnostic
    ;; pointing at accept specifically. Pass timeout-ms to raise a
    ;; clear, catchable error instead of blocking forever when no
    ;; connection arrives within it. socket-accept itself (srfi106.c,
    ;; via net_accept_with_timeout in network_internal.h) does the real
    ;; work: a non-blocking-accept retry loop against a single absolute
    ;; deadline, not a separate readiness-poll-then-blocking-accept, so
    ;; there's no TOCTOU window for a reset connection to defeat the
    ;; bound.
    (define (ws-accept listener . maybe-timeout-ms)
      (let* ((socket (if (pair? maybe-timeout-ms)
                          (socket-accept (%ws-listener-socket listener) (car maybe-timeout-ms))
                          (socket-accept (%ws-listener-socket listener))))
             (in (socket-input-port socket))
             (out (socket-output-port socket))
