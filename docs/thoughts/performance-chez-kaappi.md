...
| 2.5 | Unchecked primitive variants under dominating-test analysis (`OP_CAR_NOCHECK`, `OP_ADD_FX`) | Chez §3.5 — **step 1 (open-coding) shipped for car/cdr/cons/pair?/null?** (branch `tier2-unchecked-primitives-1`, 2026-08-24): found on starting this item that its own prerequisite didn't exist — `car`/`+`/etc. were never open-coded at all before this landing, always compiled as an ordinary `OP_CALL_GLOBAL` to the primitive procedure; the doc's own earlier claim that curry "already open-codes these as opcodes" didn't match reality (`OP_CAR`/`OP_CDR`/`OP_CONS`/`OP_NULLP`/`OP_PAIRP` existed in `opcode.h` with real VM handlers but zero compiler emitters). Repurposes those five previously-dead opcodes: each takes one const-pool operand (the callee symbol), validated at *every* execution against the actual `prim_car`/`prim_cdr`/etc. C function pointer (not a compile-time-captured value — a real bug found and fixed mid-landing, since a value captured after an earlier redefinition would wrongly treat the redefinition as the primitive forever) via the same `glob_cache` infrastructure `OP_LOAD_GLOBAL`/`OP_CALL_GLOBAL` already use, falling back to an ordinary call on a mismatch. A second real bug, found by independent code review: naively open-coding a *tail-position* call broke proper tail-call optimization for a redefined name (the fallback path recurses via `apply_arr`→a nested `vm_run()`, unlike `OP_TAIL_CALL_GLOBAL`'s frame-reuse) — fixed by never open-coding a call already known to be in tail position at all. Benchmarked (Release, low-variance): ~5.7% faster on a car/cdr-heavy traversal — the expected narrow scope of open-coding alone, removing call-frame setup but not the type check itself. **Step 2 (open-coding extended to `+`/`-`/`*`/`=`/`<`/`<=`/`>`/`>=`) also shipped** (same branch, 2026-08-25): repurposes `OP_ADD`/`OP_SUB`/`OP_MUL`/`OP_NUMEQ`/`OP_LT`/`OP_LE`/`OP_GT`/`OP_GE` (all previously dead code, zero emitters, same as step 1's five) the same way — one const-pool operand, guarded against the real `prim_add`/`prim_num_lt`/etc. C function pointer, strictly 2-arg only (R7RS's own variadic `+`/`-`/`*`/comparison chains fall through to the ordinary call path unchanged, since the opcodes themselves are strictly binary). `OP_EQ` (a separate, still-genuinely-dead legacy alias that used to share one switch-case body with `OP_NUMEQ` via fallthrough) was split out and left untouched rather than folded into this. Design correction made *before* writing the fast path, not after: an earlier plan to reuse `OP_ADD`/`OP_LT`/etc.'s own separate, pre-existing inline fixnum-fast-path bodies as the "match" branch was dropped on finding that `prim_num_lt`/`le`/`gt`/`ge`'s real 2-arg case dispatches to `sx_lt`/`sx_le`/`sx_gt`/`sx_ge` for a symbolic (CAS) operand — logic the raw opcodes' existing fixnum-or-`num_lt` bodies never replicated, so reusing them for an open-coded `(< x 5)` on a sym-var `x` would have silently returned the wrong thing instead of a real symbolic comparison node. Fixed by calling the real primitive function directly on a match instead, for all eight operators uniformly (same as car/cdr/cons/pair?/null?'s own design). Caught one self-inflicted test bug while landing this (not a product bug): a new test that redefines `<`/`+` via `run_vm_script` runs *before* an unrelated new symbolic-dispatch test in the same test function, and both defines and later calls share ONE real, process-wide `GLOBAL_ENV` across the whole `curry_test` binary — the redefinition permanently poisoned `<`/`+` for every later check in the same process until the test itself was reordered (symbolic-dispatch checks now run first). 324/324 C unit tests, 107/107 ctest suites, fresh `--clear-cache` run. **Step 3 (the actual dominating-test analysis + true `_NOCHECK` variants) attempted, then reverted** (same branch, 2026-08-25) — recorded here in detail so a future attempt doesn't re-tread the same path blind. Built `OP_CAR_NOCHECK`/`OP_CDR_NOCHECK`: a new compiler pass (`ir_emit_dominating_car_cdr`, `ir_emit`'s `IR_IF` case) recognized the exact syntactic shape `(if (pair? x) (car x) ...)` / `(if (pair? x) (cdr x) ...)` — the then-branch had to be *exactly* that call, so by construction nothing runs between the `pair?` test succeeding and the branch evaluating — and fused it into a single opcode that re-verifies `pair?` *and* `car`/`cdr` are still the real, unredefined primitives (via `glob_cache`) at the moment the branch executes, skipping the `vis_pair` check itself (not just call-frame setup) on a match. Two real problems surfaced, one fixed, one fatal to the whole approach as designed:
  1. **Fixed**: the first version emitted the fused opcode without ever emitting code to push the guarded variable's value onto the VM stack — `OP_CAR_NOCHECK`'s `POP()` silently read whatever value happened to already be on top of the stack from unrelated earlier code and dereferenced it as a pair unchecked. Trivially reachable by ordinary code (`lib/curry/modules/curry/conditions.scm`'s own `make-condition` uses exactly this shape); it read wrong condition fields silently and then SIGSEGV'd downstream in `scm_string_append` on corrupted data. Invisible to a narrow manual smoke test that only checked return values looked plausible — only caught by running the *full* test suite (18 of 107 `ctest` suites failed, several with real SIGSEGVs) and bisecting against a `git stash`ed baseline to confirm it wasn't pre-existing flakiness.
  2. **Not fixed, led to the revert**: independent code review found the soundness argument ("nothing runs between the test and the branch, so re-verifying now proves it was true then too") only holds for single-threaded execution. `GLOBAL_ENV` is shared across curry's actor threads (`src/actors.c`) with atomic version-based visibility — nothing stops another thread from mutating `pair?`/`car` in the narrow window between one thread's `OP_JUMP_FALSE` and its `OP_CAR_NOCHECK`/`OP_CDR_NOCHECK`. Concretely: thread A evaluates `(if (pair? x) (car x) ...)` with `x` not a pair, while `pair?` is (adversarially) redefined to always return `#t` — the test passes on the lie. If thread B concurrently *restores* the real `pair?`/`car` before thread A reaches the branch, thread A's re-check now finds both "real," trusts the skip, and does an unchecked `vcar` on a non-pair — reading garbage instead of raising, exactly the failure mode the soundness argument claimed was impossible. Narrow (requires code already violating `pair?`'s contract, raced against a same-window restore) but genuine, and not something a small local fix closes — it would need version-check plumbing threading proof from the test opcode through to the branch opcode, not just a bigger runtime check.
  Combined with a separate, independent finding — **benchmarked (Release, 5 runs each, car/cdr-heavy loop) and found no measurable win over step 1 alone**, averaging ~2% *slower* with fusion enabled than with it forced off, consistently across two 5-run batches. Root cause understood, not just observed: soundness required *adding* a second `glob_cache` identity check (verifying `pair?`, on top of the check `OP_CAR`/`OP_CDR` already do for themselves) to skip only a single, cheap, well-predicted `vis_pair` branch inside `prim_car`/`prim_cdr` — the added verification cost and the removed check are roughly the same size, so there was no net win in this shape even before the concurrency problem. Given zero measured benefit *and* a genuine (if narrow) soundness gap under this codebase's actor model, the cost/benefit case for keeping the added opcodes/compiler-pass complexity was negative — reverted in full (`opcode.h`/`chunk.c`/`vm.c`/`compiler.c`/`tests/test_core.c` all restored to their step-2 state) rather than shipped with a caveat. **2.5 stands at step 1+2 shipped; step 3 (true unchecked variants) is open again**, with the above as a concrete account of one path that doesn't work, for whoever picks it up next — a viable version would need to either avoid the actor-shared-global-mutation threat model (e.g. a `_NOCHECK` variant only valid for provably-non-escaping/non-redefinable local bindings, sidestepping `glob_cache` re-verification entirely) or prove the version-check plumbing pays for itself, which this attempt did not attempt to build. |
| 2.6 | Point `src/llvm/codegen.cpp` at the IR instead of raw S-exprs; add `opt -passes=verify` on every emitted module in Debug/CI | Kaappi §4.3 — **verify-pass half already done** (found already unconditional, not just Debug/CI, at all four `codegen_*` entry points — stronger than this item asked for, nothing further needed). **IR-retargeting half: step 1 only** (branch `tier2-unchecked-primitives-1`, 2026-08-24): exposes `ir_lower()`+`ir_optimize()` as a public entry point (`compiler_ir_lower_for_jit`, compiler.h) — real groundwork, but NOT on its own something `codegen.cpp` (or anything else) can walk standalone yet, since the IR is lazily lowered by design: `IR_VAR_REF` resolution is deferred to `ir_emit` time, and a lambda's body stays raw, un-lowered S-expression until `ir_emit` walks it one statement at a time. `codegen.cpp` itself (~1400 lines) is still a complete, independent second special-form dispatcher matching raw S-expressions directly (`emit_if`/`emit_lambda`/`emit_and`/`emit_or`/`emit_cond`/`emit_do`/`emit_call`, its own `collect_free_vars`), intertwined with GC statepoint insertion the header's own comment calls "prohibitive" to retrofit once a backend is working — retargeting it to walk IR node kinds instead, correctly, without silently breaking GC safety under statepoints, is a genuinely large rewrite deliberately deferred as its own separately-scoped project, not attempted here. **Planning restarted 2026-08-25** — see `docs/thoughts/tier2-6-llvm-ir-retargeting-plan-2026-08-25.md` for the working plan, tracked across sessions there rather than in this table cell. That plan reframes the item: the original "make the IR eager" framing above turned out not to be the real blocker (`codegen.cpp` already has fully independent scope-tracking that never needed the VM's eager resolution) — the actual prerequisite is an interleaved lower-then-consume adapter mirroring `ir_emit`'s own pattern, substantially smaller in scope than a full eagerness rearchitecture. |
