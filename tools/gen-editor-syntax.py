...
and emits four artifacts that stay in sync with the runtime:
  ...
  modules/lsp/symbols_gen.h            (hover/completion table for the LSP
                                        server — special forms, builtins,
                                        and Akkadian synonym -> canonical
                                        name mappings)
...
def c_str(s: str) -> str:
    """C string literal, UTF-8 source-encoded (cuneiform passes through)."""
    return '"' + s.replace("\\", "\\\\").replace('"', '\\"') + '"'


def emit_lsp_header(pools) -> str:
    """modules/lsp/symbols_gen.h — static tables for hover/completion.

    Kept minimal on purpose: name + kind is enough for the first LSP
    milestone (hover text, completion labels). Docstrings can be layered
    ...
    lines = [
        ...
        "typedef struct { const char *syn; const char *canon;"
        " const char *kind; } LspSyn;",
        "",
        "static const LspSym lsp_forms[] = {",
    ]
    for name in pools["forms"]:
        lines.append(f'    {{{c_str(name)}, "special form"}},')
    lines.append("};")
    lines.append(f"static const int lsp_forms_n = {len(pools['forms'])};")
    lines.append("")

    lines.append("static const LspSym lsp_builtins[] = {")
    for name in pools["builtins"]:
        lines.append(f'    {{{c_str(name)}, "builtin procedure"}},')
    lines.append("};")
    lines.append(f"static const int lsp_builtins_n = {len(pools['builtins'])};")
    lines.append("")

    lines.append("static const LspSyn lsp_akk_forms[] = {")
    for eng, tr, cu in akk_sf:
        if any(ch.isspace() for ch in tr) or any(ch.isspace() for ch in cu):
            continue
        lines.append(f'    {{{c_str(tr)}, {c_str(eng)}, "special form"}},')
        lines.append(f'    {{{c_str(cu)}, {c_str(eng)}, "special form"}},')
    lines.append("};")
    lines.append(f"static const int lsp_akk_forms_n = "
                  f"(int)(sizeof(lsp_akk_forms)/sizeof(lsp_akk_forms[0]));")
    lines.append("")

    lines.append("static const LspSyn lsp_akk_builtins[] = {")
    for eng, tr, cu in akk_pr:
        if any(ch.isspace() for ch in tr) or any(ch.isspace() for ch in cu):
            continue
        lines.append(f'    {{{c_str(tr)}, {c_str(eng)}, "builtin procedure"}},')
        lines.append(f'    {{{c_str(cu)}, {c_str(eng)}, "builtin procedure"}},')
    lines.append("};")
    lines.append(f"static const int lsp_akk_builtins_n = "
                  f"(int)(sizeof(lsp_akk_builtins)/sizeof(lsp_akk_builtins[0]));")
    lines.append("")
    lines.append("#endif /* CURRY_LSP_SYMBOLS_GEN_H */")
    return "\n".join(lines) + "\n"
...
def main():
    ...
    outputs = {
        ...
        EDITORS / "vscode" / "syntaxes" / "curry.tmLanguage.json":
            emit_tm(pools),
        ROOT / "modules" / "lsp" / "symbols_gen.h": emit_lsp_header(pools),
    }
