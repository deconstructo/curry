...
| [sql](module-sql.md) | `(curry sql)` | Cross-database layer (sqlite today; mariadb/postgres reserved) — one `?`-placeholder style, alist rows, `sql-with-transaction` *(pure Scheme, no build step)* | `(curry sqlite)`, `(curry conditions)` |
