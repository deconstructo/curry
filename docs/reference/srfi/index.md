...
| [`(srfi s28 format)`](s28.md) | [SRFI-28](https://srfi.schemers.org/srfi-28/) | `format` — `~a`/`~s`/`~%`/`~~` directives, plus SRFI-29's `~N@*` positional-reference extension (one shared format engine for both SRFIs) |
| [`(srfi s29 localization)`](s29.md) | [SRFI-29](https://srfi.schemers.org/srfi-29/) | Locale-sensitive message bundles — `current-language`/`current-country`/`current-locale-details`, `declare-bundle!`, `localized-template`, with fallback search |
...
| *no import — built into core `cond`* ([`s61.md`](s61.md)) | [SRFI-61](https://srfi.schemers.org/srfi-61/) | Extended `cond` arrow clause, `(generator guard => receiver)` — implemented directly in the compiler/evaluator, not a library shim (`cond` is a hardcoded special form; see `s61.md`) |
...
| [`(srfi s78 lightweight-testing)`](s78.md) | [SRFI-78](https://srfi.schemers.org/srfi-78/) | `check`/`check-set-mode!`/`check-reset!`/`check-passed?`/`check-report` lightweight inline testing — `'off` mode never evaluates `expr` at all, matching the reference implementation; `check-ec` (the SRFI-42 comprehension variant) not implemented, curry has no SRFI-42 |
...
| [`(srfi s95 sorting)`](s95.md) | [SRFI-95](https://srfi.schemers.org/srfi-95/) | `sort`/`sort!`/`merge`/`merge!`/`sorted?` — `(sequence less? [key])` argument order, over lists and vectors; a thin shim over `(srfi s132 sorting)`'s real merge-sort logic |
...
| [`(srfi s208 nan-procedures)`](s208.md) | [SRFI-208](https://srfi.schemers.org/srfi-208/) | Dissect/construct IEEE-754 NaN sign/quiet/payload bits — `make-nan`, `nan-negative?`, `nan-quiet?`, `nan-payload`, `nan=?`; backed by two new core bit-pun primitives, `%flonum-raw-bits`/`%raw-bits->flonum` |
...
| [`(srfi s212 aliases)`](s212.md) | [SRFI-212](https://srfi.schemers.org/srfi-212/) | `define-alias` — works for aliasing an ordinary value/procedure; can't alias a macro or one of curry's hardcoded special forms (same class of limit as SRFI-61, see `s212.md`) |
| [`(srfi s215 log)`](s215.md) | [SRFI-215](https://srfi.schemers.org/srfi-215/) | Central log exchange |
| [`(srfi s225 dictionaries)`](s225.md) | [SRFI-225](https://srfi.schemers.org/srfi-225/) | Generic key/value dictionary interface (Dictionary Type Objects) over alists and curry's own hash tables — `dict-ref`, `dict-set!`, `dict-fold`, `make-dto`, etc. |
