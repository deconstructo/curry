...
(define (compute-reference-orbit!)
  (when (or (not *orbit-cx*)
    ...
    (let* ((iters (inexact->exact *max-iter*))
      ...
      ;; Iterate orbit; track how many steps are valid (stops early if ref escapes)
      (let loop ((zx-hi 0.0) (zx-lo 0.0) (zy-hi 0.0) (zy-lo 0.0) (n 0))
        (if (>= n iters)
            (set! *orbit-len* iters)             ; interior — all iters valid
            (let ((base (* 16 n)))
              ...
              (if (>= (+ (* zx-hi zx-hi) (* zy-hi zy-hi)) 4.0)
                  (set! *orbit-len* (+ n 1))     ; exterior — orbit escaped at n
