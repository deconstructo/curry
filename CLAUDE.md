...
- **Raw GLSL shaders** (`make-gl-shader`, `gl-shader-draw!`): General-purpose GPU shader API. The OpenGL context is requested as Core Profile 3.3 at startup (4.1 on macOS, 3.3–4.6 on Linux). HiDPI is handled automatically — `gl-shader-draw!` auto-sets `u_resolution` (physical px) and `u_dpr` so shaders can convert logical zoom to physical pixels. `(make-gl-shader frag-src)` or `(make-gl-shader vert-src frag-src)` — lazy compilation on first draw. `(gl-shader-draw! prog painter uniforms-alist)` binds the shader, sets uniforms, draws a fullscreen quad via `gl_VertexID` (no VBO needed), then calls `endNativePainting()` so QPainter is fully restored for HUD overlays. Uniform dispatch: fixnum→`int`, bool→`int`, pair-of-2/3/4→`vec2/3/4`, else→`float`. Default vertex shader renders a fullscreen quad using `gl_VertexID`.
